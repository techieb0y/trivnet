...
if ( !has_session($mycall) ) {
	...
	$q = "SELECT DISTINCT callsign, dest, message, to_timestamp(timestamp) AS timestamp, symbol, tactical FROM messages WHERE( dest='$mycall' OR dest='all' OR callsign='$mycall') ORDER BY timestamp DESC LIMIT $num";
