...
if ( !has_session($mycall) ) {
	...
	$q = "SELECT messages.callsign, messages.dest, messages.message, to_timestamp(messages.timestamp) AS timestamp, sessions.symbol, sessions.tactical FROM messages,sessions WHERE messages.callsign=sessions.callsign AND ( dest='$mycall' OR dest='all') AND sessions.sessionid NOT LIKE 'VOID%' ORDER BY timestamp DESC LIMIT $num";
